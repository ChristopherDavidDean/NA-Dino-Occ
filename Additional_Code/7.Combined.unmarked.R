...
################################################################################
#            FILE 7: RUNNING COMBINED OCCUPANCY MODELS IN UNMARKED             #
################################################################################
...
target <- "Ceratopsidae"
samp_val <- 20
bins <- c("teyen", "teyeo", "teyep", "teyeq")
bins <- c("teyen", "teyeo") # MAAS
bins <- c("teyep", "teyeq") # CAMP
bins <- c("teyeo", "teyep", "teyeq")
...
master.occs.binned.targeted$mid_ma <- (master.occs.binned.targeted$max_ma+master.occs.binned.targeted$min_ma)/2
...
##### OCCUPANCY DATA #####
# Load data 
bin.prep <- function(bins){
  all.sitecov <- data.frame()
  all.survcov <- data.frame()
  for(i in bins){
    temp <- read.csv(paste("Prepped_data/Occurrence_Data/", bin.type, "/", i, "/", 
                            res, "/", i ,".", res, ".", target, ".dframe.", samp_val,
                            ".csv", sep = "")) 
    temp$X <- paste(temp$X, i, sep = "")
    
    tempcol <- read.csv(paste("Prepped_data/Occurrence_Data/", bin.type, "/", i, "/", 
                              res, "/", i ,".", res, ".", target, ".colframe.", samp_val,
                              ".csv", sep = ""))
    tempcol$X <- paste(tempcol$X, i, sep = "")
    ...
    survcov$bin <- NA
    survcov[rowSums(is.na(survcov))==1,5] <- i
    
    all.sitecov <- rbind(all.sitecov, sitecov)
    all.survcov <- rbind(all.survcov, survcov)
    if(i == bins[[1]]){
      all.y <- y
      all.col <- tempcol
    }else{
      all.y <- rbind(all.y, y)
      all.col <- rbind(all.col, tempcol)
    }
  }
  assign("sitecov", all.sitecov, env = .GlobalEnv)
  assign("survcov", all.survcov, env = .GlobalEnv)
  assign("y", all.y, env = .GlobalEnv)
  assign("colframe", all.col, env = .GlobalEnv)
}

bin.prep(bins)

look.up <- master.occs.binned.targeted %>%
  select(collection_no, mid_ma) %>%
  distinct()

for (i in 2:ncol(colframe)) {
  colframe[, i] <- look.up$mid_ma[match(colframe[, i], look.up$collection_no)]
}
colframe <- colframe[,-1]
obsCovs <- list(age = colframe)
...
# Fit full model
full <- occu(formula =  ~ OUTa + MGVF + TEMP + RAIN + SEDf + LAND + age # det
               ~ Ppre + Ptem + BINS, # occ
               data = umf)

# Use dredge to automatically carry out model selection
(modelList <- dredge(full, rank = "AICc")) 
...
dir.create(paste("Results/Unmarked/", bin.type, "/combined/", sep = ""), showWarnings = FALSE)
...
write.csv(combined.res, paste0("Results/Unmarked/", bin.type, "/combined/", target, 
                               ".", paste(bins, collapse = "."),".combined.results.", 
                               res, ".", samp_val, ".csv", sep =""))
# Average model table
write.csv(temp.model.res, paste0("Results/Unmarked/", bin.type, "/combined/", target, 
                                 ".", paste(bins, collapse = "."), ".model.averaged.covs.betas.", 
                                 res, ".", samp_val, ".csv", sep =""))
# Full model list
write.csv(modelList, paste0("Results/Unmarked/", bin.type, "/combined/",
                            target, ".", paste(bins, collapse = "."), 
                            ".full.mod.table.", res, ".", 
                            samp_val, ".csv", sep =""))
...
pred.AGE <- seq(min(obsCovs[[1]], na.rm = TRUE), max(obsCovs[[1]], na.rm = TRUE),,100)

p.DEMs <- (pred.DEMs - means[1]) / sds[1] # Standardise them like actual covs
p.RAIN <- (pred.RAIN - means[2]) / sds[2]
p.RAIN <- scale(pred.RAIN)
p.MGVF <- scale(pred.MGVF)
...
# Obtain predictions
newData <- data.frame(age = 0, SEDf = 0, RAIN = 0, OUTa = 0, MGVF = p.MGVF)
...
plot(pred.det.RELI[[1]] ~ pred.MGVF, type = "l", lwd = 3, col = "blue", ylim = c(0,1), las = 1, ylab = "Pred. detection prob.", xlab = "Relief", frame = F)
matlines(pred.RAIN, pred.det.RELI[,3:4], lty = 1, lwd = 1, col = "grey")
