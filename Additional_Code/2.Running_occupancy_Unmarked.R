...
# Occupancy
Phot.orig <- sitecov[,"hot_mean"]
Pcol.orig <- sitecov[,"col_mean"]
Pwet.orig <- sitecov[,"wet_mean"]
Pdry.orig <- sitecov[,"dry_mean"]
Pann.orig <- sitecov[,"ann_sd"]
...
siteCovs <- data.frame(DEMs = DEMs.orig, OUTc = OUTc.orig,
                       ...
                       Phot = Phot.orig, Pcol = Pcol.orig, 
                       Pwet = Pwet.orig, Pdry = Pdry.orig, 
                       Pann = Pann.orig, COLC = COLC.orig)
...
umf@siteCovs$Phot <- scale(umf@siteCovs$Phot)
umf@siteCovs$Pcol <- scale(umf@siteCovs$Pcol)
umf@siteCovs$Pdry <- scale(umf@siteCovs$Pdry)
umf@siteCovs$Pwet <- scale(umf@siteCovs$Pwet)
umf@siteCovs$Pann <- scale(umf@siteCovs$Pann)
...
# Histograms
par(mfrow = c(1,1))
for(i in 1:ncol(siteCovs[-c(8,16)])){
  hist(siteCovs[-c(8,16)][,i], breaks = 50, col = "grey", main = colnames(siteCovs[-c(8,14)])[i])
}

# Correlation plot
corrplot(cor(siteCovs[-c(8,16)]), method = "number")

# Multicollinearity
testmodel <- lm(Relief ~ mean_DEM + mean_prec + mean_temp + colls + AllOut + MGVF +
                  MOut + COut + wet_mean + dry_mean + col_mean + hot_mean + ann_sd, data = sitecov)
...
if(bin == "teyen" | bin == "teyeo"){
  full <- occu(formula =  ~ OUTm + MGVF + RAIN + LAND + SEDf   # det
               ~ Pcol + Phot + Pdry, # occ
               data = umf)
} else{
  full <- occu(formula =  ~ OUTc + MGVF + RAIN + LAND + SEDf   # det
               ~ Pcol + Phot + Pdry, # occ
               data = umf)
}
